...
#include "/public/home/sunpeng/zhangxin/INSTALL/latticechina/inc/dslash.h"
//#define _time_debug_
...
namespace Chroma
{
    ...
    public:
        ...
        Real rho;
        Real kappa;
        double *chroma_gauge[4];
        int *site_vec;
        int *subgs;
        std::vector<double *> hw_evec;
        std::vector<double> hw_eval;
        std::vector<std::vector<double>> hw_coef;
        std::vector<int> hw_size;
        void *OW;
        multi1d<LatticeColorMatrix> u2;
        Real get_rho(void) { return rho; }
        OverlapEigenOperator(GroupXML_t &fermact, multi1d<LatticeColorMatrix> &u, int neig = 0) : EigenOperator<LatticeFermion>(fermact, u, neig), u2(u)
        {
            ...
            if (Noeigen_hw > 0)
            {
                if (TheNamedObjMap::Instance().check(HW_eigen))
                {
                    ...
                    {
                        QDPIO::cerr << "Size of " << HW_eigen << " is not large enough (" << Hw->size() << " v.s. " << Noeigen_hw << ")." << std::endl;
                        QDP_abort(1);
                    }
                    read(paramtop, "accuracy", accuracy);
                    read(paramtop, "topocharge_para", topocharge_para);
                    QDPIO::cout << "read param finished" << std::endl;
                    fflush(stdout);
                }
                else
                {
            ...
            {
                chroma_gauge[0] = (double *)&(u[0].elem(0).elem());
                chroma_gauge[1] = (double *)&(u[1].elem(0).elem());
                chroma_gauge[2] = (double *)&(u[2].elem(0).elem());
                chroma_gauge[3] = (double *)&(u[3].elem(0).elem());
                site_vec = (int *)(&Layout::lattSize()[0]);
                subgs = (int *)(&Layout::subgridLattSize()[0]);
                hw_evec.resize(Noeigen_hw);
                hw_eval.resize(Noeigen_hw);
                hw_coef.resize(_chey_size_);
                hw_size.resize(_chey_size_);
                //                EigenOperator<LatticeFermion> &es=*Hw;
                for (int i = 0; i < Noeigen_hw; i++)
                {
                    double *spinorIn = (double *)&(es[i].vec.elem(0).elem(0).elem(0).real());
                    hw_evec[i] = spinorIn;
                    hw_eval[i] = toDouble(real(es[i].val));
                }
                for (int i = 0; i < _chey_size_; i++)
                {
                    hw_coef[i] = coef[i];
                    hw_size[i] = coef[i].get_HwSize();
                }
            }
            OW = newApplyWilsonQuda(
                chroma_gauge[0], chroma_gauge[1], chroma_gauge[2], chroma_gauge[3],
                site_vec[0], site_vec[1], site_vec[2], site_vec[3],
        ...
        void HwSq_scaled_test(LatticeFermion &chi, const LatticeFermion &psi, double cut) const
        {
            T tmp1, tmp2;
            EigenOperator<LatticeFermion> &es = *Hw;
            es(tmp1, psi, PLUS);
        ...
        void general_dov(LatticeFermion &chi, const LatticeFermion &psi, double k0, double k1, double k2, double prec) const
        {
...
#endif
            {
                ...
                for (int i = 0; i < coef[is].get_HwSize(); i++)
                {
                    ...
                    //				QDPIO::cout<< " i =  " << i  << " inner =  " << inner  <<  " innerProduct(es[i].vec, es[i].vec) =  " << innerProduct(es[i].vec, es[i].vec)  << std::endl;
                ...
                //	QDPIO::cout<< " coef[is].size() =  " << coef[is].size()  << std::endl;
                for (int i = coef[is].size() - 1; i >= 1; i--)
                {
                    if (i < coef[is].size() - 1)
                        HwSq_scaled(high, *pbn1, cut_sq_tmp);
                    *pbn2 = 2.0 * high - *pbn0 + coef[is][i] * tmp;
                    //				*pbn2 = 2.0*high - *pbn0 + coef[is][coef[is].size()-1]*tmp;//check
                    ptmp = pbn0;
                    pbn0 = pbn1;
                    pbn1 = pbn2;
                    pbn2 = ptmp;
                ...
                //		                es(tmp1,*pbn1,PLUS);
                //		                es(tmp2,tmp1,PLUS);
                //				HwSq_scaled(high,*pbn1,cut_sq_tmp);//check
                HwSq_scaled(high, *pbn1, cut_sq_tmp);
                //			*pbn1 = high - *pbn0;
                *pbn1 = high - *pbn0 + coef[is][0] * tmp;
                ...
                ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                {
                    ...
                    double *src = (double *)&(psi.elem(0).elem(0).elem(0).real());
                    double *dest = (double *)&(chi2.elem(0).elem(0).elem(0).real());
                    ApplyWilsonQuda(dest, src, OW);
                    snoop.stop();
                    ...
                    D.create(fs, anisoParam);
                    D(tmp1, psi, PLUS);
                    QDPIO::cout << "(tmp1 chi2) dest_Chroma =  " << norm2(tmp1) << " dest_Wilson =  " << norm2(chi2 * (-2)) << " norm2(dest_Chroma-dest_Wilson)/norm2(dest_Chroma) = " << norm2(tmp1 - chi2 * (-2)) / norm2(tmp1) << std::endl;
                    double time_gpu = snoop.getTimeInSeconds();
                    QDPIO::cout << "time_gpu=   " << time_gpu << std::endl;
                    LatticeFermion chi3 = chi2;
                    LatticeFermion chi1 = chi;
                    QDPIO::cout << " sour =  " << norm2(psi) << " chi =  " << norm2(chi1) << " chi2 =  " << norm2(chi3) << " norm2(chi1-chi2) = " << norm2(chi1 - chi3) / norm2(chi3) << std::endl;
                }
            }
        }
...
>>>>>>> ba63a7a30c9743e8edd2f6352b781c74903a4687
